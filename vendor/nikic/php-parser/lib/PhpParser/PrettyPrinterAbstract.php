...
use Jacoby\Intervention\PhpParser\Internal\DiffElem;
use Jacoby\Intervention\PhpParser\Internal\PrintableNewAnonClassNode;
use Jacoby\Intervention\PhpParser\Internal\TokenStream;
...
abstract class PrettyPrinterAbstract
{
        ...
        foreach ($node->getSubNodeNames() as $subNodeName) {
            ...
            if (!$subNode instanceof Node && $subNode !== null || !$origSubNode instanceof Node && $origSubNode !== null) {
                ...
                if (\is_array($subNode) && \is_array($origSubNode)) {
                    ...
                    $listResult = $this->pArray($subNode, $origSubNode, $pos, $indentAdjustment, $subNodeName, $fixupInfo[$subNodeName] ?? null, $this->listInsertionMap[$type . '->' . $subNodeName] ?? null);
                    if (null === $listResult) {
                        return $this->pFallback($fallbackNode);
                    }
                    $result .= $listResult;
     ...
     */
    protected function pArray(array $nodes, array $origNodes, int &$pos, int $indentAdjustment, string $subNodeName, $fixup, $insertStr)
    {
    ...
    {
        return !($node instanceof Node\Name || $node instanceof Expr\Variable || $node instanceof Expr\ArrayDimFetch || $node instanceof Expr\FuncCall || $node instanceof Expr\MethodCall || $node instanceof Expr\StaticCall || $node instanceof Expr\Array_);
    }
    /**
    ...
    {
        return ($modifiers & Stmt\Class_::MODIFIER_PUBLIC ? 'public ' : '') . ($modifiers & Stmt\Class_::MODIFIER_PROTECTED ? 'protected ' : '') . ($modifiers & Stmt\Class_::MODIFIER_PRIVATE ? 'private ' : '') . ($modifiers & Stmt\Class_::MODIFIER_STATIC ? 'static ' : '') . ($modifiers & Stmt\Class_::MODIFIER_ABSTRACT ? 'abstract ' : '') . ($modifiers & Stmt\Class_::MODIFIER_FINAL ? 'final ' : '');
    }
    /**
    ...
    protected function initializeFixupMap()
    {
        ...
        $assignOps = [Expr\Assign::class, Expr\AssignRef::class, AssignOp\Plus::class, AssignOp\Minus::class, AssignOp\Mul::class, AssignOp\Div::class, AssignOp\Concat::class, AssignOp\Mod::class, AssignOp\BitwiseAnd::class, AssignOp\BitwiseOr::class, AssignOp\BitwiseXor::class, AssignOp\ShiftLeft::class, AssignOp\ShiftRight::class, AssignOp\Pow::class];
