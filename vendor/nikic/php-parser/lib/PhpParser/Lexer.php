...
use Jacoby\Intervention\PhpParser\Parser\Tokens;
class Lexer
{
    protected $code;
    protected $tokens;
    ...
    public function __construct(array $options = [])
    {
        ...
        // value (here "true")
        $options += ['usedAttributes' => ['comments', 'startLine', 'endLine']];
        $this->usedAttributes = \array_fill_keys($options['usedAttributes'], \true);
    }
    /**
            ...
            if (\is_string($token)) {
                ...
                if (\T_COMMENT === $token[0] || \T_DOC_COMMENT === $token[0]) {
                    if (isset($this->usedAttributes['comments'])) {
                        $comment = \T_DOC_COMMENT === $token[0] ? new Comment\Doc($token[1], $this->line, $this->filePos, $this->pos) : new Comment($token[1], $this->line, $this->filePos, $this->pos);
