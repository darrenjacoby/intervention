    ...
    public function exportObject(object $object, array $path, array $parentIds) : array
    {
        ...
        foreach ($parentIds as $parentId) {
            if (isset($this->visitedObjects[$parentId][$id])) {
                throw new ExportException(\sprintf('Object of class "%s" has a circular reference at %s. ' . 'Circular references are currently not supported.', \get_class($object), ExportException::pathToString($this->visitedObjects[$parentId][$id])), $path);
            }
