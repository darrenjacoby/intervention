...
use Jacoby\Intervention\Brick\VarExporter\ExportException;
...
use Jacoby\Intervention\PhpParser\NodeTraverser;
use Jacoby\Intervention\PhpParser\NodeVisitor\FindingVisitor;
use Jacoby\Intervention\PhpParser\NodeVisitor\NameResolver;
use Jacoby\Intervention\PhpParser\Parser;
use Jacoby\Intervention\PhpParser\ParserFactory;
...
class ClosureExporter extends ObjectExporter
{
     ...
     * @throws ExportException
     */
    private function getClosure(ReflectionFunction $reflectionFunction, array $ast, string $file, int $line, array $path) : Node\Expr\Closure
    {
        $finder = new FindingVisitor(function (Node $node) use($line) : bool {
            return ($node instanceof Node\Expr\Closure || $node instanceof Node\Expr\ArrowFunction) && $node->getStartLine() === $line;
        });
        ...
        if ($count !== 1) {
            throw new ExportException(\sprintf('Expected exactly 1 closure in %s on line %d, found %d.', $file, $line, $count), $path);
        }
     ...
     * @return Node\Expr\Closure
     */
    private function convertArrowFunction(ReflectionFunction $reflectionFunction, Node\Expr\ArrowFunction $arrowFunction) : Node\Expr\Closure
    {
        $closure = new Node\Expr\Closure([], ['arrow_function' => \true]);
        ...
        foreach (\array_keys($static) as $var) {
            ...
            $closure->uses[] = new Node\Expr\ClosureUse(new Node\Expr\Variable($var));
        }
        return $closure;
    }
    /**
     ...
     * @throws ExportException
     */
    private function closureHandleUses(ReflectionFunction $reflectionFunction, Node\Expr\Closure $closure, array $path) : void
    {
        if (!$this->exporter->closureSnapshotUses) {
            $message = $closure->hasAttribute('arrow_function') ? "The arrow function uses variables in the parent scope, this is not supported by default" : "The closure has bound variables through 'use', this is not supported by default";
        ...
        foreach ($closure->uses as $use) {
            ...
            $expr = $nodes[0];
            $assign = new Node\Expr\Assign(new Node\Expr\Variable($var), $expr->expr);
