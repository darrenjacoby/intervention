...
namespace Jacoby\Intervention\Illuminate\Support\Traits;

use CachingIterator;
use Closure;
use Exception;
use Jacoby\Intervention\Illuminate\Contracts\Support\Arrayable;
use Jacoby\Intervention\Illuminate\Contracts\Support\Jsonable;
use Jacoby\Intervention\Illuminate\Support\Arr;
use Jacoby\Intervention\Illuminate\Support\Collection;
use Jacoby\Intervention\Illuminate\Support\Enumerable;
use Jacoby\Intervention\Illuminate\Support\HigherOrderCollectionProxy;
use JsonSerializable;
use Jacoby\Intervention\Symfony\Component\VarDumper\VarDumper;
use Traversable;
...
trait EnumeratesValues
{
    ...
    public function value($key, $default = null)
    {
        if ($value = $this->firstWhere($key)) {
            return \Jacoby\Intervention\Illuminate\Support\data_get($value, $key, $default);
        }
    ...
    public function whereIn($key, $values, $strict = \false)
    {
        ...
        return $this->filter(fn($item) => \in_array(\Jacoby\Intervention\Illuminate\Support\data_get($item, $key), $values, $strict));
    }
    /**
     * Filter items by the given key value pair using strict comparison.
     *
    ...
    {
        return $this->filter(fn($item) => \Jacoby\Intervention\Illuminate\Support\data_get($item, $key) < \reset($values) || \Jacoby\Intervention\Illuminate\Support\data_get($item, $key) > \end($values));
    }
    /**
     * Filter items by the given key value pair.
     *
    ...
    {
        $values = $this->getArrayableItems($values);
        return $this->reject(fn($item) => \in_array(\Jacoby\Intervention\Illuminate\Support\data_get($item, $key), $values, $strict));
    }
    /**
     * Filter items by the given key value pair using strict comparison.
     *
    ...
    protected function getArrayableItems($items)
    {
        ...
        return match (\true) {
            ...
            $items instanceof Jsonable => \json_decode($items->toJson(), \true),
    ...
    protected function operatorForWhere($key, $operator = null, $value = null)
    {
        ...
        return function ($item) use($key, $operator, $value) {
            $retrieved = \Jacoby\Intervention\Illuminate\Support\data_get($item, $key);
    ...
    protected function valueRetriever($value)
    {
        ...
        return fn($item) => \Jacoby\Intervention\Illuminate\Support\data_get($item, $value);
    }
    /**
