...
use Jacoby\Intervention\Illuminate\Support\HigherOrderWhenProxy;
use JsonSerializable;
use Jacoby\Intervention\ReturnTypeWillChange;
...
trait EnumeratesValues
{
    ...
    public function containsStrict($key, $value = null)
    {
        if (\func_num_args() === 2) {
            return $this->contains(function ($item) use($key, $value) {
                return \Jacoby\Intervention\Illuminate\Support\data_get($item, $key) === $value;
            });
        }
    ...
    public function whereIn($key, $values, $strict = \false)
    {
        ...
        return $this->filter(function ($item) use($key, $values, $strict) {
            return \in_array(\Jacoby\Intervention\Illuminate\Support\data_get($item, $key), $values, $strict);
        });
    }
    /**
     * Filter items by the given key value pair using strict comparison.
    ...
    public function whereNotBetween($key, $values)
    {
        return $this->filter(function ($item) use($key, $values) {
            return \Jacoby\Intervention\Illuminate\Support\data_get($item, $key) < \reset($values) || \Jacoby\Intervention\Illuminate\Support\data_get($item, $key) > \end($values);
        });
    }
    /**
     * Filter items by the given key value pair.
    ...
    protected function valueRetriever($value)
    {
        ...
        return function ($item) use($value) {
            return \Jacoby\Intervention\Illuminate\Support\data_get($item, $value);
        };
    }
    /**
